...
class UserViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.signup("test1", "test1@test.com", "password", None)
        self.u1_id = 778
        self.user1.id = self.u1_id

        self.user2 = User.signup("test2", "test2@test.com", "password", None)
        self.u2_id = 884
        self.user2.id = self.u2_id

        self.user3 = User.signup('abc', 'test3@test.com', 'password', None)
        self.u3_id = 995
        self.user3.id = self.u3_id
    ...
    def setup_likes(self):
        m1 = Message(text="trending warble", user_id=self.u1_id)
        m2 = Message(text="Eating some lunch", user_id=self.u1_id)
        m3 = Message(id=9876, text="likable warble", user_id=self.u2_id)
        db.session.add_all([m1, m2, m3])
        db.session.commit()

        l1 = Likes(user_id=self.u1_id, message_id=9876)
    ...
    def test_add_like(self):
        m = Message(id=1984, text="The earth is round", user_id=self.u2_id)
        db.session.add(m)
        db.session.commit()
    ...
    def test_remove_like(self):
        ...
        # Now we are sure that user1 likes the message "likable warble"
    ...
    def setup_followers(self):
        ...
        f2 = Follows(user_being_followed_id=self.u2_id, user_following_id=self.u1_id)
        f3 = Follows(user_being_followed_id=self.u1_id, user_following_id=self.u3_id)
